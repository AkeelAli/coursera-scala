  ...
  def descendingByRetweet: TweetList = {
    def order(set: TweetSet, list: TweetList): TweetList = {
      if (set.isEmpty) list
        ...
        val leastRetweetedTweet = set.leastRetweeted
        order(set.remove(leastRetweetedTweet), new Cons(leastRetweetedTweet, list))
      }
    }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = {
    def googlefilter(x: Tweet): Boolean = {
      if (google.exists(x.text.contains)) true
      else false
    }
    
    TweetReader.allTweets.filter(googlefilter)
  }
  lazy val appleTweets: TweetSet = {
    def applefilter(x: Tweet): Boolean = {
      if (apple.exists(x.text.contains)) true
      else false
    }
    
    TweetReader.allTweets.filter(applefilter)
  }
